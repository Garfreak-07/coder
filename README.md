...
Web UI 支持选择项目文件夹、常驻项目树、底部需求输入、默认 agent 工作流、导入 workflow/agent JSON，以及拖拽组合 agent 的安全预览。项目树默认显示至少两层目录，点击或拖拽文件夹可以聚焦查看上层/下层内容。
...
```powershell
langgraph-coder --repo . --workflow-spec examples/workflows/coding-review.json --graph-only
```
