...
class RoundWorkingSet(BaseModel):
    ...
    context_packet_refs: dict[str, str] = Field(default_factory=dict)
    native_runtime_refs: dict[str, list[str]] = Field(default_factory=dict)
    diff_refs: dict[str, list[str]] = Field(default_factory=dict)
    log_refs: dict[str, list[str]] = Field(default_factory=dict)
    ...
    def record_context_packet_ref(self, work_item_id: str, ref: str) -> str:
        self.context_packet_refs[work_item_id] = ref
        return ref
    ...
    def record_native_runtime_ref(self, work_item_id: str, ref: str) -> str:
        return self._record_ref(self.native_runtime_refs, work_item_id, ref)

    def record_diff_ref(self, work_item_id: str, ref: str) -> str:
        return self._record_ref(self.diff_refs, work_item_id, ref)

    def record_log_ref(self, work_item_id: str, ref: str) -> str:
        return self._record_ref(self.log_refs, work_item_id, ref)
    ...
    def runtime_refs_for_work_item(self, work_item_id: str) -> dict[str, list[str]]:
        return {
            "native_runtime_refs": list(self.native_runtime_refs.get(work_item_id, [])),
            "diff_refs": list(self.diff_refs.get(work_item_id, [])),
            "log_refs": list(self.log_refs.get(work_item_id, [])),
        }
    ...
    def _record_ref(self, target: dict[str, list[str]], work_item_id: str, ref: str) -> str:
        refs = target.setdefault(work_item_id, [])
        if ref not in refs:
            refs.append(ref)
        return ref


__all__ = ["RoundWorkingSet"]
