...
class RefOnlyDataPlaneTests(unittest.TestCase):
    def test_agent_graph_normal_events_do_not_embed_large_payload_keys(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            result = AgentGraphRunner(default_planner_led_agent_workflow()).run(
                "Check compact events.",
                tmp,
            )
...
def _first_banned_key(value: Any, prefix: str = "") -> str | None:
    if isinstance(value, dict):
        for key, child in value.items():
            ...
            if key_text in BANNED_EVENT_KEYS:
                return path
