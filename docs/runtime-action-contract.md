...
Runtime actions are low-level effects requested by worker artifacts and executed
only through `ActionGateway`. They are compact, auditable records for Planner,
not a direct worker-to-tool escape hatch.

## Request Shape

Worker artifacts may include `requested_actions` entries with:
...
- `requires_permission` / `requires_approval`: explicit worker-side risk hints.
...
`ActionGateway` again. Replay must not re-run the worker model that originally
...
- `WorkflowRunner`, legacy `WorkflowSpec`, and legacy artifacts are not valid
  product runtime-action paths.
