...
| Extensions/plugins | `extensions/*` | `coder-extensions` plugin registry | Rust plugin/harness-runtime manifest types, builtin plugin manifests, and external-effect preview validation are in place; install/search/execute API parity remains later | External-effect approval and manifest tests pass |
