...
| Repo evidence tools | `context/repo_*`, Git inspection helpers | `coder-tools` repo search/read/status/diff/find plus `coder-store` repo evidence refs | Port path safety, sensitive-path filtering, bounded line-range reads, bounded previews, and evidence write behavior | Existing repo_search/read/discovery/status/diff and evidence-store tests have Rust equivalents |
