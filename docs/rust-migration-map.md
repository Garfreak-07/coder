...
| Repo evidence tools | `context/repo_*`, Git inspection helpers | `coder-tools` repo search/read/status/diff/find | Port path safety, sensitive-path filtering, bounded line-range reads, bounded previews, and evidence write behavior | Existing repo_search/read/discovery/status/diff tests have Rust equivalents |
