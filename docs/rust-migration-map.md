...
| ActionGateway/tool execution | `actions/*` | `coder-tools`, `coder-sandbox`, Rust tool gateway | Ported command preview plus patch preview/apply API paths with side-effect eventing; Rust `coder-harness` now mirrors plugin operation approval policy and MCP manifest validation | Patch/command/plugin/MCP policy tests pass in Rust |
