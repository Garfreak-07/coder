...
matrix coverage, and legacy isolation.
...
User interaction remains `User <-> Planner`. Worker, Tester, Synthesizer, and
Final Tester agents return structured artifacts, blockers, or evidence to
Planner and never ask the user directly.
...
- Product live AgentGraph runs do not compile `WorkflowSpec`, run
  `WorkflowRunner`, or emit legacy artifacts outside legacy preview.
...
- Ordinary UI exposure of runtime JSON, `ContextPolicy`, `TokenBudget`, manual
...
2. Runtime action audit/replay: close silent drops and add Planner-approved
   replay without worker re-execution.
...
5. Legacy isolation: keep compatibility preview explicit and deprecated while
